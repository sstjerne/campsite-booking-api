...
public class BookingCRUDTest {
	...
	/***
	 * Simple CRUD : Creates one booking of 2 days and then it change for 3 days, it should be update properly. Finally
	...
	public void shouldCreateRetrieveDelete() throws Exception {
		...
		customer.setEmail("customer@test1.com");
		customer.setFullname("Customer Test1");
		...
		// CREATE
		MvcResult result = mvc.perform(post("/booking/").content(bookingJson).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isCreated()).andReturn();
		String bookingId = Util.getResourceIdFromUrl(result.getResponse().getRedirectedUrl());

		// RETRIEVE
		mvc.perform(get("/booking/" + bookingId).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(jsonPath("$.id", is(bookingId))).andExpect(jsonPath("$.confirmed", is(true)))
				.andExpect(jsonPath("$.checkIn", is(booking.getCheckIn().toString())))
				.andExpect(jsonPath("$.checkOut", is(booking.getCheckOut().toString())))
				.andExpect(jsonPath("$.campsiteId", is(Integer.valueOf(campsiteId))))
				.andExpect(jsonPath("$.customer.email", is(booking.getCustomer().getEmail())))
				.andExpect(jsonPath("$.customer.fullname", is(booking.getCustomer().getFullname())));
		...
		// UPDATE
		result = mvc.perform(put("/booking/" + bookingId).content(bookingJson).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
		...
		// DELETE
		mvc.perform(delete("/booking/" + bookingId)).andExpect(status().isNoContent());

		// RETRIEVE should fail
		mvc.perform(get("/booking/" + bookingId).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound());
	}

	/***
	 * Creates three booking of 1 , 2 & 3 days. It should throw error of 4 days. Finally the booking are cancelled.
	 * 
	 * @throws Exception
	 */
//	@Test
	public void shouldBeThereAvailability() throws Exception {
		...
		LocalDate from2 = to1.plusDays(1);
		...
		LocalDate from3 = to2.plusDays(1);
		...
		LocalDate from4 = to3.plusDays(1);
		LocalDate to4 = from4.plusDays(3);
		booking.setCheckIn(from4);
		booking.setCheckOut(to4);

		writeValueAsString = objectMapper.writeValueAsString(booking);
		System.out.println(writeValueAsString);
		byte[] bookingJson4 = writeValueAsString.getBytes();

		// CREATE
		result = mvc.perform(post("/booking/").content(bookingJson4).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().is4xxClientError()).andReturn();

		// RETRIEVE
		mvc.perform(get("/booking/").contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
